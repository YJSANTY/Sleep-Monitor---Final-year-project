...
<html>
  <head>
    <title>SLEEP WITH MYSQL DATABASE</title>
  ...
  <body>
    ...
    <h3 style="color: #0c6980;">Sleep RECORD DATA TABLE</h3>
    ...
    <table class="styled-table" id= "table_id">
      ...
      <tbody id="tbody_table_record">
        <?php
          ...
          $num = 0;
          //------------------------------------------------------------ The process for displaying a record table containing the max sensor data 
          $pdo = Database::connect();
          // replace_with_your_table_name, on this project I use the table name 'sleep_table_max_leds_record'.
          // This table is used to store and record MAX sensor data updated by ESP32. 
          ...
          $sql = 'SELECT * FROM sleep_table_max_leds_record ORDER BY date, time';
          foreach ($pdo->query($sql) as $row) {
            ...
            echo '<td class="bdr">'. $row['SpO2'] . '</td>';
            echo '<td class="bdr">'. $row['Heart_Beat'] . '</td>';
            echo '<td class="bdr">'. $row['Noise'] . '</td>';
	    echo '<td class="bdr">'. $row['Body_Motion'] . '</td>';
